...
{
	static final int DEFAULT_PORT = 6000;//端口号
	static ServerSocket serverSocket;//聊天服务器的套接字
	static Vector<Socket> connections;// 连接数组
	static Vector<ClientProc> clients;//客户端数组
	...
	public static void sendAll(String s)
	{
		if (connections != null)
		{
			for (Enumeration e = connections.elements(); // 可以用for each循环
			e.hasMoreElements();)
			{
				try
				{
